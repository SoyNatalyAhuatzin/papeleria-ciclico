...
// const bcrypt = require('bcrypt'); // Uncomment to enable password hashing
const app = express();
const port = process.env.PORT || 3001;

// const saltRounds = 10; // For bcrypt, uncomment if using
      ...
      if (password.trim() === user.password_hash) {
        ...
        if (userRol === rol) {
          ...
          const rolePages = {
            ...
            contador: '/contadorabilidad.html',
            cajero: '/cajero.html',
          };
...
app.post('/api/usuario', (req, res) => {
  ...
  const query = `
    ...
    VALUES (?, ?, ?, ?, NOW())
  `;
  console.log('Ejecutando query:', query, [nombre.trim(), email.trim(), password.trim(), rol]);
  db.query(query, [nombre.trim(), email.trim(), password.trim(), rol], (err, results) => {
    if (err) {
      console.error('Error al crear usuario:', err.message, 'Código:', err.code, 'SQLState:', err.sqlState);
      if (err.code === 'ER_DUP_ENTRY') {
        return res.status(400).json({ error: 'El correo ya está registrado' });
      }
      return res.status(500).json({ error: `Error en el servidor: ${err.message} (Código: ${err.code})` });
    }
...
app.get('/api/inventario/detalle', (req, res) => {
  const query = `
    ...
    GROUP BY 
      p.producto_id, p.sku, p.nombre, p.descripcion, c.nombre, pr.nombre, 
      p.precio_compra, p.precio_venta, p.precio_especial, p.unidad_medida
  `;
  db.query(query, (err, results) => {
    if (err) {
      console.error('Error en la consulta de inventario detallado:', err.message);
      return res.status(500).json({ error: 'Error en el servidor' });
    }
    console.log('Inventario detallado enviado:', results.length);
    res.json(results);
  });
});
